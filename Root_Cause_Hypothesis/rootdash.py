...
def root_cause_analysis_panel():
    ...
    st.markdown(
    "Now that we have identified the <span style='color: #1b9e4b;'><strong>prioritized touchpoints</strong></span>, this tool serves as a <strong>reference base</strong> for internal brainstorming sessions aimed at validating <span style='color: #1b9e4b;'><strong>39</strong></span> potential <span style='color: #1b9e4b;'><strong>root causes</strong></span> that have already been mapped across the 11 touchpoints of the process. It organizes these causes into specific touchpoints, allowing you to explore the relationships between these touchpoints and stakeholders. By using a dynamic table, it makes it easier to visualize and analyze all the possible root causes.",
    ...
    - <span style='color:#1b9e4b;'><strong>1 - Attend Online Consultation</strong></span> (62.3%)
    - <span style='color:#1b9e4b;'><strong>2 - Leave Review and Feedback</strong></span> (25.08%)
    - <span style='color:#1b9e4b;'><strong>3 - Search and Evaluate Professional Score</strong></span> (1.65%)
    ...
    *Note: The "Search and Evaluate Professional Score" touchpoint represents a strategic action designed to exert influence and improve overall engagement on the platform.*
    ...
    with col1:
        ...
        - <span style='color: #1b9e4b;'><strong>Touchpoint</strong></span>: The process. <span style='color: #1b9e4b'>({len(df['Touchpoint'].unique())})</span>
        - <span style='color: #1b9e4b;'><strong>Possible Process Issues</strong></span>: Potential process problems. <span style='color: #1b9e4b'>({len(df['Possible Process Issues'].unique())})</span>
        - <span style='color: #1b9e4b;'><strong>Symptoms</strong></span>: How the problems are perceived by the patient. <span style='color: #1b9e4b'>({len(df['Symptoms'].unique())})</span>
        - <span style='color: #1b9e4b;'><strong>Effect of Symptom</strong></span>: Impact of symptoms on the patient experience. <span style='color: #1b9e4b'>({len(df['Effect of Symptom'].unique())})</span>
        - <span style='color: #1b9e4b;'><strong>Possible Causes</strong></span>: Possible causes of the problems. <span style='color: #1b9e4b'>({len(df['Possible Causes'].unique())})</span>
        - <span style='color: #1b9e4b;'><strong>Root Causes</strong></span>: Ultimate level factors. <span style='color: #1b9e4b'>({len(df['Reasons'].unique())})</span>
        - <span style='color: #1b9e4b;'><strong>Process Owner</strong></span>: Oversees the process and drives continuous improvements. <span style='color: #1b9e4b'>({len(df['Stakeholder'].unique())})</span>
        """, unsafe_allow_html=True)
